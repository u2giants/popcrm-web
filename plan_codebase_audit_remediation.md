...
| 2 | Opportunity Chat CRM authorization | complete | App commits `4c66402` (authorization) and `4f9a9a4` (Node 20 user-client transport fix) pushed to `main`; Kimi K3 reviewed both diffs and returned `APPROVE` after all findings were fixed. 5 files/31 Vitest tests, lint (only pre-existing `src/App.tsx:48` warning), build, worker/helper syntax, and `git diff --check` passed. Shared-db guards and Build and Deploy runs `30300857606` / `30303290580` passed. After exact owner authorization, production `popcrm-fireflies` was restarted on checkout/worker commit `4f9a9a4`, health returned `{"ok":true}`, missing and invalid tokens returned 401, a real DAM-only user with CRM access revoked returned 403, and the active CRM test user passed authorization and reached safe request validation (400 `invalid_request` for a deliberately invalid UUID). No opportunity rows were available for a non-invented 200 chat probe. |
...
| 6 | Durable Outlook Graph ingestion | complete | Shared-db PR #801 merged as `326c25e`; migration `20260812010000` was preview-proven, Kimi-approved, then promoted alone to production by guarded run `31613800275`. App commit `3abaf61` is pushed and deployed by green run `31614092763`; 111 tests, lint (only the pre-existing `src/App.tsx:48` warning), build, syntax, and diff checks passed. Kimi returned `APPROVE` after the same agent fixed stale-writer replay, safe error handling, early config validation, rebuild totals, and contract-wiring coverage. Production checks: initial delta sync handled 50 changes over 6 pages and saved the cursor; immediate resume handled 1 safe duplicate over 1 page; the timer's post-enable run handled 3 changes including 2 tombstones and saved the cursor. Timer is active/waiting for the next 15-minute run. |
| 7A | Shared-db Overview/sidebar aggregate contracts (preview-proven) | complete | Shared-db PR #848 merged; migrations `20260812130000` and `20260812211000` preview-proven on `rjyboqwcdzcocqgmsyel`, then promoted to production `qsllyeztdwjgirsysgai` by guarded run `31662233425` after Albert's exact in-chat approval on 2026-08-13. Review evidence run `31662213719`, digest `sha256:818278bb…ceea`. Post-apply catalog verification clean; all seven `api.crm_overview_*` overloads present with EXECUTE held by `authenticated` only. Displayed-value inventory: `docs/overview-aggregate-inventory.md` (app commit `0885628`). Result recorded on shared-db issue #851. Two dispatches failed first without touching production: the digest must be canonical `sha256:<64 hex>`, and shared-db `main` moved between recording evidence and dispatching. |
| 7B | Frontend integration after authorized merge/vendor sync | complete | App commits `5443870`, `094275e`, `ef4c31e` pushed and deployed by green run `31664788133`. `useCrmStatsQuery`'s seven unbounded fetches replaced by seven independent hooks on shared `crmKeys.stats()` keys. **Numeric proof:** old client path vs new contracts, run against production as the same authenticated CRM user — 49 of 49 displayed values identical (8 scalar KPIs, 6 donut buckets, 8 stages, 24 chart buckets, 3 recent-row panels by id and order). The old path was pulling 8,655 contact rows every 90 seconds. GLM 5.2 reviewed to consensus: it withdrew its two headline defects once shown `api.ts:36`, and found a real one — affirmative empty states ("Inbox zero") rendering during load and during an email outage, fixed in `094275e`. |
| 8 | Dependency and Recharts remediation | complete | App commits `bde0ded`, `f94fbb9`. `npm audit` and `npm audit --omit=dev` both report 0 vulnerabilities, from 12 (1 critical, 7 high): react-router-dom 7.18.2, vitest 4.1.10, recharts 3.10.1, plus transitive fixes via `npm audit fix` with no `overrides`. `shadcn` moved to devDependencies — it is NOT unused: `src/index.css:3` imports `shadcn/tailwind.css` and removing the package breaks the build. Unused `src/components/ui/chart.tsx` deleted rather than migrated. Visually verified in a real browser at 1440px and 390px via the new credential-free probe at `/__charts` (`src/dev/ChartProbe.tsx`), zero console errors; probe strings confirmed absent from the production bundle. |
| 9 | Deployment shell cleanup | complete | App commit `70beeec`, green run `31693044569`. Removed exactly the seven orphaned lines (an `exit 0`, an unmatched `fi`, an unmatched `done`) after the real failure `exit 1`. It had never gone red because bash parses lazily and the step exits on its success path first. Added `scripts/check-workflow-shell.sh`, which extracts every multi-line `run:` block and parses it with `bash -n`, proven against the pre-fix file, hardened to fail if it extracts zero blocks, and wired into CI. `docs/deployment.md` updated. |
| 10 | Auth refresh race | complete | App commits `bf2e3c0`, `1cd7c4c`, green run `31694206708`. `src/auth/refreshGate.ts` applies a profile result only if it is the newest refresh AND its session user is still current; `SIGNED_OUT`, `logout()` and unmount reject in-flight refreshes synchronously. A failed `current_user_profile` is still non-fatal but is now logged, and impersonation is erased when the real account is not an administrator. GLM 5.2 found that StrictMode's mount/unmount/remount left the gate permanently disposed and the app stuck signed out; the effect now creates a fresh gate per run. 9 gate tests plus provider-level tests under happy-dom, and the StrictMode test provably fails with the fix reverted. |
| 11 | CI executes tests | complete | Delivered with Session 9 in app commit `70beeec`. The `verify` job now runs `./scripts/check-workflow-shell.sh`, `npm run lint`, `npm test` and `npm run build` as four separately named steps before `build-and-push`, so the Actions UI shows which gate failed. Confirmed on run `31693044569`: all four steps present and green, then image and deploy, then the exact-SHA production check. `docs/deployment.md` and `docs/development.md` updated. `paths-ignore` unchanged. |
| 12 | Auth callback lint/performance warning | complete | App commit `948c630`, green run `31694686853`. The gate now reads the callback error as lazy state instead of setting it from an effect, and the effect only strips error material from the URL when this load arrived with some. `readAuthCallbackError` extracted to `src/auth/callbackError.ts` so it is testable without instantiating the Supabase client; 8 tests cover query, hash, the friendly unprovisioned-user message and its two near misses, code-without-description, and query-over-hash precedence. `npm run lint` is now zero errors and zero warnings, and `AGENTS.md`'s accepted-warnings row is replaced with that fact rather than left as history. No lint rule was suppressed. |
