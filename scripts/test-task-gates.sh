...
TMP="$(mktemp -d)"; trap 'rm -rf "$TMP"' EXIT; cd "$ROOT"
...
expect_class src/lib/supabase.ts ui-live-workflow 'browser authentication target requires authenticated visual proof'
expect_class workers/crm-worker-supabase.mjs reviewer-safety 'privileged CRM worker receives protected full treatment'
...
expect_class systemd/example.service deployment 'host worker units are protected deployment work'
...
expect_class src/lib/types.ts shared-db 'application database contract retains the governed route'

fixture="$TMP/popcrm-web"; git -C "$TMP" init --quiet popcrm-web
git -C "$fixture" config user.name 'Task Gate Test'; git -C "$fixture" config user.email 'task-gate-test@example.invalid'
git -C "$fixture" remote add origin https://github.com/example/popcrm-task-gate-fixture.git
mkdir -p "$fixture/.ai-devops"; cp "$ROOT/.ai-devops/task-gates.json" "$fixture/.ai-devops/task-gates.json"
git -C "$fixture" add .ai-devops/task-gates.json; git -C "$fixture" commit --quiet -m baseline
(
  cd "$fixture"; ai-task-gates start --class code --base HEAD >/dev/null
  mkdir -p supabase/migrations; printf '%s\n' '-- fixture' > supabase/migrations/fixture.sql
)
...
assert_blocked 'owner request cannot bypass protected database work' --owner-request 'fixture owner request'
(
  cd "$fixture"; rm -rf supabase; mkdir -p tools; printf '%s\n' 'export const fixture = true;' > tools/fixture.ts
  ai-task-gates start --class code --base HEAD >/dev/null; ai-task-gates check --before ship >/dev/null
)
pass 'valid code flow proceeds to shipping checks'
cp "$fixture/.ai-devops/task-gates.json" "$TMP/policy.backup.json"; rm -f "$fixture/.ai-devops/task-gates.json"
printf '%s\n' 'src/lib/types.ts' > "$TMP/rollback-path"
